...
{
    public sealed class CollDownDecorator<TContext> : DecoratorBase<TContext>, IGuard<TContext>
    {
        ...
        public CollDownDecorator(float coolDown, INode<TContext> child, string? name = null) : base(child, name ?? "CoolDown")
        {
            if (coolDown <= 0)
    ...
    public static class CollDownExtension
    {
            ...
            float coolDown, Action<ISingleChild<TContext>> buildAction, string? name = null)
        {
            ...
            buildAction(builder);
            return b.Add(builder.Build((cd, c, n) => new CollDownDecorator<TContext>(cd, c, n)));
        }
             ...
             float coolDown, Action<ISingleChild<TContext>> buildAction, string? name = null)
        {
            var builder = new DecoratorBuilder<float, TContext>(coolDown, name);
            buildAction(builder);
            b.Set(builder.Build((cd, c, n) => new CollDownDecorator<TContext>(cd, c, n)));
        }
    }
}